...
//!
//! Bench small N (≤32) with tpg=32 to see if single-simdgroup
//! dispatch avoids the second-level threadgroup reduction overhead.
...
// ── small N: tpg=32, single simdgroup, no second-level reduction ──────────
#[bench_kernel(
    op="softmax",
    subop="softmax_small_n",
    class=RowNorm,
    b=1024,
    n=32,
    tpg=32,
...
#[kernel]
pub fn mt_softmax_small_n<T>(inp: Tensor<T>, out: Tensor<T>, #[constexpr] n: u32) {
    let row = program_id::<0>();
    let rs = row * n;
    let re = rs + n;
    let nf = n / (lsize * 4u32);
